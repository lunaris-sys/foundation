...
Settings        & System-wide configuration: theming, permissions, connections, AI level \\
...
\paragraph{Shell API.}
Modules access the same \texttt{shell} namespace as first-party applications,
with two differences. First, the available surface is restricted to what the
module declared in its manifest: a module without \texttt{graph.allow = "read"}
cannot call \texttt{shell.graph.query}, and a module without
\texttt{clipboard.read.sensitive} receives \texttt{null} for sensitive
clipboard entries. Second, modules access the API via
\texttt{@os/module-sdk} rather than os-sdk directly, which wraps the same
underlying interfaces with module-specific permission enforcement.

The full Shell API surface -- clipboard, presence, search, annotations,
intents, timeline, shortcuts, badges, context menu, onboarding, ambient,
spatial, and voice -- is documented in Section~\ref{sec:shell-api}. Modules
may use any surface for which they hold the corresponding manifest permission.

The following surfaces are always available to modules without additional
permission declarations, as they carry no privacy or security implications:

\begin{lstlisting}[language=bash, caption={Module SDK -- always available surfaces}]
...
// Badges (always available, error/warning types logged to graph)
shell.badges.set({ count: 3 })
shell.badges.clear()

// Ambient effects (respects user's global ambient preference)
shell.ambient.set({ effect: "pulse", intensity: 0.2, reason: "syncing" })
shell.ambient.clear()
\end{lstlisting}
...
\subsection{MCP Safety and External Connections}
\label{sec:mcp-safety}

The MCP Server Layer enables powerful integrations between the AI daemon,
first-party applications, and external tools such as n8n and OpenClaw. Two
mechanisms exist to prevent this power from causing runaway or uncontrolled
behavior: a call depth limit for MCP chains, and a controlled outbound webhook
surface for the Event Bus.

\paragraph{Call depth limit.}
Every MCP call chain carries a \texttt{call\_chain\_id} and a \texttt{depth}
counter. When a component initiates an MCP call the depth starts at one. If
that call triggers a further MCP call, the depth increments and the chain
identifier is forwarded. A call that would exceed depth five is rejected
immediately with the error \texttt{mcp\_depth\_exceeded} rather than
executing. The full call chain is written to the Audit Log on rejection so
the user can diagnose what triggered the loop.

The default limit of five is intentionally conservative. Legitimate nested
MCP calls rarely exceed two or three levels. Users who build complex
multi-step workflows can raise the limit to a maximum of ten in Settings under
AI; no configuration can exceed ten regardless of how it is set. The limit
applies globally across all MCP callers including the AI daemon, modules, and
external connections.

\paragraph{Outbound Event Bus webhooks.}
The Event Bus can forward selected events to external endpoints configured as
Connections in Settings (Section~\ref{sec:connections}). This enables tools
like n8n to react to desktop events without polling. The mechanism is
intentionally restrictive to prevent uncontrolled data leakage.

Outbound webhook endpoints are bound to a Connection. There is no mechanism
for arbitrary URL registration outside of a Connection; a module or a
\texttt{.project} file cannot register a webhook endpoint on its own. The
Event Bus receives its permitted endpoint whitelist directly from the Account
Daemon at startup and after any Connection change. Any event targeting an
endpoint not on the whitelist is silently discarded without error or retry.

The user selects which event types to forward per Connection from a fixed list
derived from the Event Bus schema. Raw eBPF events are not available as
outbound event types regardless of configuration; only semantic events such as
\texttt{focus.activated}, \texttt{timeline.record}, and
\texttt{issue.status\_changed} can be forwarded. This prevents low-level
system telemetry from leaking to external services even if the user
misconfigures a connection.

Outbound events are rate-limited to 60 per minute per Connection. When the
limit is reached the remaining events in that minute are dropped, not queued,
and a warning badge appears on the Connection in Settings. The limit resets
at the start of the next minute. This prevents high-frequency desktop activity
from generating uncontrolled external traffic volume.

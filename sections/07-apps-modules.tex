...
The \texttt{shell.focus} callbacks fire after all system-level Focus Mode
changes have been applied, so the project context is consistent when the
module receives the event. A Waypointer provider that narrows its result set
to the active project subscribes to \texttt{onActivate} and resets on
\texttt{onDeactivate}.

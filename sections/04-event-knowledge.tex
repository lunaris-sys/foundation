...
validates and forwards to the Graph Writer Daemon.}
...
higher-level \texttt{UserAction} nodes are derived from clusters of related
events and represent meaningful activity rather than individual syscalls.

\paragraph{Event ingestion tiers.}
Not all event sources are equal in structure or volume, and the ingestion
strategy reflects that. Three tiers operate independently.
...
written directly to the graph with no intermediate processing step. This tier
handles the large majority of graph writes and is the only tier with hard
latency requirements; writes are batched as described in
Section~\ref{sec:backpressure} but no interpretation step sits between the
event and the graph.

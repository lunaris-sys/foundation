...
\subsection{Wayland as an Event Source}
\label{sec:wayland-events}

The Wayland compositor sits at the intersection of every user interaction with
the graphical desktop. Because the compositor is a first-party component of
this project, it can emit structured events to the Event Bus directly without
any protocol extension or third-party cooperation.

Wayland provides the following event categories natively, without requiring
applications to do anything:

\emph{Window lifecycle.} The compositor knows when a Wayland surface is
created, mapped (becomes visible), unmapped, and destroyed. This translates
directly to app open and close events carrying the app ID, window title, and
timestamp. The \texttt{xdg-shell} protocol provides this for all standard
application windows; \texttt{wlr-foreign-toplevel-management} extends it to
allow the shell to observe all open windows including those from other clients.

\emph{Focus transitions.} The compositor controls which surface receives
keyboard input at any given moment. Every focus change (the user switching
windows, clicking into a different app, or the active window closing) is an
event the compositor observes before any application does. Focus events carry
the incoming and outgoing app IDs and the timestamp, giving the graph an
accurate record of how attention moves across applications.

\emph{Clipboard operations.} The Wayland clipboard protocol
(\texttt{wl\_data\_device}) routes all clipboard interactions through the
compositor. The compositor observes when content is placed on the clipboard and
when a paste request is made, including which application is the source and
which is the destination. Clipboard \emph{content} is never logged; only the
operation metadata (source app, destination app, MIME type, timestamp) reaches
the Event Bus.

\emph{Output events.} Monitor connect, disconnect, and configuration changes
are compositor-level events. These are logged as session context, useful for
queries like ``which monitor was I on when I worked on this file.''

What Wayland does \emph{not} provide, and what therefore requires eBPF or
app-level instrumentation: filesystem access, network connections, what the
user typed, the content of windows, or any in-application state beyond the
window title. The compositor sees surfaces, not their contents. This is by
design: it is the same isolation property that makes Wayland more secure than
X11, and it defines the boundary between what the compositor can contribute
and what other event sources must cover.
...
at runtime, so no compiler is required on the end-user's machine. \texttt{aya}
supports \emph{CO-RE} (Compile Once, Run Everywhere)~\cite{ebpf:core}, which
uses BTF type information embedded in the kernel to make eBPF programs portable
across kernel versions without recompilation. The result is a fully Rust
codebase with no unsafe FFI boundaries, no runtime compilation, and no
additional system dependencies beyond what the kernel already provides.

\paragraph{eBPF Normalizer.}
Raw eBPF events arrive at high frequency from the kernel ring buffer. Before
they reach the Event Bus they pass through the \emph{eBPF Normalizer}, a
user-space component that performs three operations in sequence.

First, \emph{deduplication}: identical operations on the same file by the same
process within 100 milliseconds are collapsed into a single event with an
updated timestamp. A text editor that calls \texttt{read()} on a file hundreds
of times per second produces one graph-relevant event, not hundreds.

Second, \emph{path filtering}: events on paths with no semantic value for a
graph of user activity are discarded entirely before reaching the bus.
Discarded prefixes include \texttt{/proc}, \texttt{/sys}, \texttt{/dev},
\texttt{/tmp}, shared libraries under \texttt{/usr/lib}, and kernel internal
paths. An event on \texttt{/proc/1234/status} carries no information the
Knowledge Graph needs; dropping it at the normalizer saves the bus, the Graph
Writer, and SQLite from processing it at all.

Third, \emph{UUID assignment}: each surviving event receives a UUID~v7
identifier. UUID~v7 embeds a millisecond-precision timestamp in its most
significant bits, making events generated close together sort together
lexicographically, a property that improves write performance and
time-range query efficiency in SQLite.

The normalizer runs in the same user-space process as the eBPF ring buffer
consumer. It introduces no IPC hop; filtered events pass directly to the
Event Bus client in the same process. Drop rates and deduplication ratios
are exposed as metrics and monitored by the Anomaly Detector.
...
\begin{tikzpicture}[
	...
	% Graph Writer (x=10.2), SQLite (x=12.8), Ladybug (x=12.8 lower)
	\node[proc]  (gw)     at (10.2, 2.25) {Graph\\Writer};
	\node[store] (sqlite) at (12.8, 2.25) {SQLite\\(writes)};
	\node[proc]  (promo)  at (12.8, -0.6) {Promotion\\pass};
	\node[store] (ladybug)   at (10.2, -0.6) {Ladybug\\(queries)};
	...
	% Bus -> Writer -> SQLite, SQLite -> Promotion -> Ladybug
	\draw[arr] (bus.east) -- (gw);
	\draw[arr] (gw)     -- (sqlite);
	\draw[arr] (sqlite) -- (promo);
	\draw[arr] (promo)  -- (ladybug);
...
moves semantically valuable nodes into Ladybug for graph traversal queries.}
...
\paragraph{Ladybug as the query store.}
\emph{Ladybug}~\cite{ladybug} is the query store: an embedded graph database written
...
process, no network port, and no additional service to manage. Ladybug uses
...
Ladybug is a community fork of \emph{Kuzu}~\cite{kuzu}, an embedded graph
database originally developed by Kùzu Inc. and based on research published at
VLDB~\cite{kuzu:vldb}. In October 2025, Kùzu Inc. was acquired by Apple and
archived the open-source Kuzu repository without a migration path. Ladybug
emerged as the primary community-maintained continuation: it carries the full
Kuzu codebase, Cypher API, and Rust bindings forward under the same MIT
license, with active releases continuing from where Kuzu left off.

This dependency carries a known risk that is acknowledged rather than
minimised. Ladybug's contributor base is small and its long-term governance is
not yet as established as projects like SQLite or DuckDB. The mitigation is
architectural: the Graph Daemon is the sole component that opens the Ladybug
database files directly. No other part of the system has a compile-time or
runtime dependency on Ladybug's API. A migration to an alternative embedded
graph database (whether DuckDB with its \texttt{duckpgq} graph extension,
a future stable fork, or a custom SQLite-based traversal layer) would be
contained entirely within the Graph Daemon and require no changes to any
consumer of the graph query interface. This isolation is a deliberate design
choice that applies the same dependency containment principle used throughout
the rest of the system.

Ladybug operates on a curated graph, not a raw event log. Raw eBPF noise never
...
their relationships. This keeps the Ladybug graph clean and traversal queries
fast.

\paragraph{Promotion pipeline.}
A background promotion pass runs periodically during idle periods, reading
from SQLite and writing promoted nodes and edges to Ladybug. The pass is
...
lookups over recent raw events; Ladybug answers multi-hop graph traversals over
...
\begin{lstlisting}[language=SQL, caption={Example Cypher query against Ladybug: files accessed last Tuesday}]
...
stream, and a promotion pipeline moves semantically valuable data into Ladybug
for graph traversal. This section describes the Ladybug graph itself: its schema,
ingestion tiers, and the data that lives in it.

\paragraph{Why not RDF and SPARQL.}
\emph{RDF} (Resource Description Framework) and \emph{SPARQL} are W3C
standards for the \emph{Semantic Web}: a vision of interlinked, publicly
queryable knowledge graphs on the open internet. They are designed for
schema-flexible, distributed, open data: a fundamentally different problem
than this system's.

This system's Knowledge Graph is local, private, schema-fixed, and
performance-sensitive. RDF's triple-store model (\texttt{subject predicate
object}) is optimized for schema flexibility at the cost of query expressiveness
and traversal performance. SPARQL is significantly less expressive than Cypher
for multi-hop graph traversals and pattern matching, which are the primary
query patterns this system needs. The reference implementations of RDF stores
(rdf4j, Apache Jena) run on the JVM, which is incompatible with the project's
Rust-first system stack and introduces runtime overhead inappropriate for an
embedded component.

W3C interoperability is not a design goal. The Knowledge Graph is an internal,
local data store; it does not need to federate with external systems or conform
to open web standards. If data export is needed, the export format is a
separate concern from the storage format and can be addressed at the export
layer without constraining the internal representation.
...
occurrence from the event pipeline lands in SQLite first; promotion to Ladybug
...
Section~\ref{sec:backpressure}; promotion to Ladybug follows asynchronously.
...
such as PDFs opened in the viewer, browser page titles, and text file contents,
a small local model running at idle can extract topic labels and named entities
and attach them as properties on the corresponding \texttt{File} or \texttt{Event}
...
Most project management tools require the user to create structure before they
can derive value from it: name the project, configure it, move the files, and
only then does the tool become useful. This inverts the actual workflow: the
work exists before the label does.

This system takes the opposite approach. The graph accumulates enough signal
from normal activity to infer that a coherent context exists, at which point
the user is asked to confirm and name it rather than construct it from scratch.
Value is immediate; the label is applied after the fact.

Explicit upfront creation is fully supported for users who prefer to define
structure before starting. The difference is that it is an option, not a
prerequisite. A project that has never been named still accumulates a full
activity history, supports AI context scoping, and surfaces correctly in
Waypointer once it is confirmed. Nothing is lost by starting without a
\texttt{.project} file.
...
# provider options: github-issues | gitlab-issues | openproject |
#                   plane | jira
...
is active: recent files, last commit message, open issues, and time spent
...
The \texttt{[tracker]} block in the \texttt{.project} file connects the
project to an external issue tracker. The Graph Daemon reads this block and
exposes the configuration to the corresponding tracker Integration Package,
which registers a Waypointer provider that surfaces issues as typed
\texttt{issue} results (Section~\ref{sec:waypointer}).

When a project is focused, typing \texttt{\#} in Waypointer opens issue
search against the configured tracker. Selecting an issue opens an inline
expansion showing title, description, status, assignee, and labels without
leaving Waypointer. From there the user can activate issue-scoped Focus Mode,
open the issue in the browser, or create a related issue with pre-filled
context.

Issue-scoped Focus Mode adds the issue as a sub-context layer on top of the
active project. The AI daemon receives the issue title, description, and
labels in its context window. When the focus session ends, a
\texttt{UserAction} node is written to the graph recording which files were
touched and the session duration, giving automatic time tracking at the issue
level without any manual input.

Supported tracker providers, each delivered as a separate Integration Package:
GitHub Issues, GitLab Issues, OpenProject, Plane, and Jira. The
\texttt{provider} field in \texttt{[tracker]} selects which package handles
the connection; the Install Daemon installs the matching Integration Package
automatically when it detects the \texttt{.project} file declares a provider.
...
The \emph{Module System} (Section~\ref{sec:apps-modules}) supports the
...
Raw eBPF events (SQLite) &
  ...
  anomaly detection window; never promoted to Ladybug and discarded
  after expiry. \\
Semantic nodes (Ladybug) &
  12 months rolling &

...
org/
  ...
  knowledge       <- SQLite write store, Kuzu query store, Graph Daemon
  ai-layer        <- provider abstraction, MCP client
  compositor      <- cosmic-comp fork (Wayland compositor)
  desktop-shell   <- taskbar, launcher, shell (Tauri + TypeScript)
  ui-kit          <- shared component library (TypeScript + Tailwind)
  os-sdk          <- shared Rust crate + TypeScript shell API for first-party apps
  app-files       <- File Manager
  app-settings    <- Settings
  app-terminal    <- Terminal
  app-store       <- Store
  install-daemon  <- Install Daemon, Integration Package matching
  module-sdk      <- shell API subset + module scaffolding for third-party modules
...
system environment. The \texttt{distro} repository contains a script
...
host directory into the VM via \emph{virtiofs}~\cite{virtiofs} (a shared
filesystem protocol that exposes a host directory to a QEMU guest at
near-native speed using the virtio transport), so that the developer can
compile on the host and test in the VM without a full image rebuild, and snapshot
...
\texttt{blueprint} repository and component repositories link back to the

...
because the Graph Daemon independently reads the Policy Store and rejects any
...
Event Bus and writes to Kuzu, and \texttt{graph-daemon-read} answers queries
from applications and daemons.

\texttt{graph-daemon-read} opens the Kuzu database in read-only mode. A
...
another compromised component. The read-only Kuzu handle prevents writing or
...
injection is an open, unsolved problem across the AI industry. No current
system can guarantee that a sufficiently sophisticated injection embedded in
external content will be detected or neutralized. This system's position is to
minimize exposure, make AI-triggered actions visible and reversible, and be
transparent about the remaining risk rather than to claim a solution that does
not exist.

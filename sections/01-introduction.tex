...
(sovereign but inaccessible). No system targeting mainstream usability currently occupies the
upper-right.}
...
The scope of this project is worth stating precisely, because several reasonable
expectations turn out not to apply here.

Complexity is intentional. A system that works for non-technical users requires
more infrastructure than a minimal one, not less. This is not a stripped-down
environment optimized for simplicity of implementation.

Reproducibility is a goal; mandatory declarative configuration is not. Users
should not need to manage configuration files to install software or change
settings. Where declarative tooling is useful under the hood, it is used, but
it is never a requirement imposed on the user.

The primary design constraint is the non-technical user switching from a
commercial system. Advanced Linux users are well-served by the architecture
and are welcome, but their preferences do not override decisions made for
accessibility and usability. Those two audiences rarely conflict, but when they
do, the newcomer wins.

Business deployment is a future consideration, not a present one. The architecture
supports managed environments, but the system is not designed as an enterprise
product. User rights are not negotiable regardless of deployment context.

The goal is a better alternative to existing systems, not a familiar imitation
of them. Compatibility bridges are provided so that existing software works, but
the interface, the architecture, and the underlying model are not constrained by
what Windows or macOS do.

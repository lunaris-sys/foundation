...
\begin{tikzpicture}[
	...
	topbox_opt/.style={
		draw, thick, dashed, rectangle, rounded corners=2pt,
		minimum width=2.5cm, minimum height=1.6cm,
		font=\small\bfseries, align=center, inner sep=6pt
	},
	...
	\node[box] (os) at (0, 0) {
		OS Base\\[2pt]
		{\mdseries\footnotesize Fedora $\cdot$ systemd $\cdot$ Btrfs $\cdot$ RPM/Flatpak}
	};
	\node[box] (de) at (0, \sep) {
		Desktop Environment\\[2pt]
		{\mdseries\footnotesize cosmic-comp $\cdot$ Shell $\cdot$ Tauri $\cdot$ ui-kit $\cdot$ os-sdk}
	};
	\node[halfbox, anchor=east] (eb) at (-0.2, 2*\sep) {
		Event Bus\\[2pt]
		{\mdseries\footnotesize Unix socket $\cdot$ protobuf}
	};
	\node[halfbox, anchor=west] (mcp) at (0.2, 2*\sep) {
		MCP Server Layer\\[2pt]
		{\mdseries\footnotesize App capability bridge}
	};
	\node[box] (kg) at (0, 3*\sep) {
		Knowledge Graph\\[2pt]
		{\mdseries\footnotesize Graph Daemon $\cdot$ SQLite $\cdot$ Ladybug $\cdot$ capability tokens}
	};
	\node[topbox]     (apps)    at (-2.975, 6.2) {
		First-party\\Applications\\[2pt]
		{\mdseries\footnotesize via os-sdk}
	};
	\node[topbox]     (daemons) at (0,      6.2) {
		System\\Daemons\\[2pt]
		{\mdseries\footnotesize Account, Health\ldots}
	};
	\node[topbox_opt] (ai)      at ( 2.975, 6.2) {
		AI Daemon\\[2pt]
		{\mdseries\footnotesize optional}
	};
	
	\draw[arr] (apps.south)    -- (kg.north -| apps);
	\draw[arr] (daemons.south) -- (kg.north);
	\draw[arr] (ai.south)      -- (kg.north -| ai);
...
(Account, Health, Audit, Install, and others), and the AI daemon (dashed, optional). All
three connect to the Graph Daemon over a Unix socket with per-application
capability tokens; none of them bypasses the daemon or accesses the graph database directly.
...
and why \emph{Ladybug}~\cite{ladybug} specifically, is explained in
...
\emph{Rust}~\cite{rust-lang}, a systems programming language developed by
Mozilla Research with its 1.0 release in 2015. Understanding why requires a
brief look at what systems programming has historically looked like.
...
issues~\cite{msrc2019memory,google:projectzero2020}.
...
\emph{Wayland}~\cite{wayland} is a display server protocol: the specification
that defines how applications communicate with the component responsible for
...
application's window~\cite{x11:security}. This is a keylogger-class capability
that cannot be removed without a protocol redesign, and it makes X11
structurally incompatible with the isolation guarantees this project requires.
...
consumes around 300\,MB of RAM at idle~\cite{electron:overhead}. On a system
with several Electron applications running, the memory cost is substantial.
...
\paragraph{Maintenance posture and divergence cost.}
Forking an actively developed project carries a real and ongoing cost that is
acknowledged directly. System76 develops cosmic-comp actively for their own
hardware and their own COSMIC desktop. Their upstream receives regular commits:
bug fixes, Smithay version bumps, new Wayland protocol extensions, and
COSMIC-specific features. Every upstream change must be evaluated and either
merged, adapted, or consciously rejected. As the fork diverges over time,
this evaluation becomes more expensive because the diff between fork and
upstream grows.

The concrete maintenance burden is: tracking upstream releases on a defined
schedule, running the integration test suite after each upstream merge,
resolving conflicts where upstream changes touch code that the fork has also
modified, and communicating breaking changes to the rest of the project when
an upstream merge requires downstream adaptation. This is not free work; it
requires at least one contributor who understands both the Smithay internals
and the fork's shell boundary well enough to evaluate upstream changes
correctly.

An alternative was considered: contributing required features upstream to
cosmic-comp rather than maintaining a fork, and using extensions or protocol
additions to achieve the shell separation. This is rejected for one concrete
reason. System76 is a commercial company with its own roadmap, hardware, and
product direction. Contributing features upstream makes them subject to
System76's review, acceptance criteria, and merge timeline. This is a
different kind of dependency, not a lighter one. It would mean the project's
compositor development velocity is gated by a company with no obligation to
prioritise or accept the contributions. The fork trades upstream merge cost
for development independence, and for a project where the compositor shell
boundary is a fundamental architectural decision, that trade is correct.

The practical mitigation for merge cost is minimising the diff. The fork does
not change window management logic, input handling, multi-monitor behaviour,
or any subsystem that cosmic-comp maintains well. Changes are concentrated at
the shell boundary: removing COSMIC-specific IPC and replacing it with
standard Wayland protocols. A smaller diff means lower merge cost and a
more tractable review process for each upstream bump.
...
Install Daemon  & Tracks app installations across all package managers; matches and installs Integration Packages automatically \\
...
\emph{Ladybug} is the query-facing graph database in this system. It is an embedded
...
Ladybug is an OLAP system: it is optimized for analytical queries over large graphs,
...
\texttt{Session} nodes -- is periodically promoted from SQLite into Ladybug, where
it becomes available for graph traversal queries. Raw eBPF noise never reaches
Ladybug; it is retained in SQLite for the anomaly detection window and then
discarded. This means Ladybug operates on a clean, curated graph rather than a
...
answers time-range lookups over recent raw events; Ladybug answers multi-hop
...
Ladybug uses \emph{Cypher} as its query language, an open graph query standard

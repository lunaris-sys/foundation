...
\begin{tikzpicture}[
	...
	\node[box] (os) at (0, 0) {
		OS Base\\[-2pt]
		{\mdseries\footnotesize Fedora/OpenSUSE $\cdot$ systemd $\cdot$ Btrfs $\cdot$ RPM/Flatpak}
	};
	\node[box] (de) at (0, \sep) {
		Desktop Environment\\[-2pt]
		{\mdseries\footnotesize Wayland (Smithay) $\cdot$ Shell $\cdot$ Tauri $\cdot$ ui-kit $\cdot$ os-sdk}
	};
...
directly interacts with: windows, the taskbar, the Waypoint launcher,
...
\emph{Smithay}~\cite{smithay}, described in Section~\ref{sec:smithay-intro}.
...
searching in Waypoint, or checking System Monitor all translate to parameterized
Cypher queries over a Unix socket. The application receives structured results
and renders them; it never touches the Kuzu files directly, and the capability
token it presents limits which node types and relations it can read.
...
Forking COSMIC was considered and rejected: it ships as an integrated product
with System76's own design language, release schedule, and branding decisions,
and the architectural overlap is not deep enough to justify the coupling that a
fork would introduce. This project's UI model, shell design, and integration
architecture diverge enough that a fork would need to undo as much as it
inherits. \emph{cosmic-comp}~\cite{cosmic:comp} serves as an implementation
reference for Smithay usage in a production compositor; it is not a base.
...
\midrule
Shell        & Taskbar, Waypoint launcher, notification center, lock screen \\

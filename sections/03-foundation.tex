...
reasons~\cite{fedora:nvidia}. The installer detects Nvidia hardware and provides
a guided path to the proprietary driver, but this is a manual step with a
restart, not a transparent first-boot experience. Broadcom Wi-Fi adapters,
found in some Apple hardware and a smaller set of laptops, require a proprietary
driver that is not included in the Fedora kernel and needs manual
installation~\cite{fedora:broadcom}. These limitations are structural to the
Linux driver ecosystem and not specific to this project; they are documented
explicitly rather than discovered by users after installation.

...
\texttt{aya} framework and ship as pre-compiled binaries in the system image.
No compiler is required on the end-user's machine. The programs use
\emph{CO-RE} (Compile Once, Run Everywhere)~\cite{ebpf:core}, a mechanism
...
stable. Kernels older than 5.8 lack ring buffer support entirely; kernels
between 5.8 and 6.5 may work but are not tested and not supported.
...
starts, networking works, and audio works without manual configuration. It does
not mean every piece of hardware on the market works automatically, and it does
not compete with the decades of work that distributions like Ubuntu have put
into hardware compatibility at scale.
...
Two known categories fall outside this guarantee. Nvidia discrete GPUs require
the proprietary driver, which Fedora does not install by default for licensing
reasons. The installer detects Nvidia hardware and provides a guided path to
the proprietary driver, but this is a manual step with a restart, not a
transparent first-boot experience. Broadcom Wi-Fi adapters, found in some
Apple hardware and a smaller set of laptops, require a proprietary driver that
is not included in the Fedora kernel and needs manual installation. These
limitations are structural to the Linux driver ecosystem and not specific to
this project; they are documented explicitly rather than discovered by users
after installation.

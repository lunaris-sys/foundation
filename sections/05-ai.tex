...
macro is a Rust community crate that enables async methods in traits, which
the stable Rust compiler does not yet support natively; it desugars the async
methods into an equivalent form that compiles correctly.
...
footprint and computation cost at a modest quality trade-off. A 7-billion
parameter model that would require roughly 28~GB of RAM at full 32-bit
precision runs in approximately 4~GB after 4-bit quantization, which falls
within the RAM budget of a typical consumer laptop. llama.cpp exposes an HTTP
API compatible with the OpenAI request format, so any client written for the
OpenAI API works against a local llama.cpp instance by changing the base URL.
...
over JSON-RPC~2.0, a lightweight remote procedure call format, typically over
a Unix socket or stdio pipe for local servers and over HTTP/SSE for remote
ones.
...
\begin{figure}[htbp]
	...
	\begin{tikzpicture}[
		...
		\node[srv] (fm)     at (7.8,  2.4) {MCP: File Manager};
		\node[srv] (term)   at (7.8,  0.8) {MCP: Terminal};
		\node[srv] (kg)     at (7.8, -0.8) {MCP: Knowledge Graph};
		\node[srv] (ext)    at (7.8, -2.4) {MCP: third-party};
		
		\draw[arr] (model) -- (client);
		
		% Bus: horizontal stub then vertical rail
		\coordinate (stub) at (5.0, 0);
		\draw[thick] (client.east) -- (stub);
		\draw[thick] (stub |- fm.west)  -- (stub |- ext.west);
		
		% Horizontal branches from rail to each server
		\draw[arr] (stub |- fm.west)   -- (fm.west);
		\draw[arr] (stub |- term.west) -- (term.west);
		\draw[arr] (stub |- kg.west)   -- (kg.west);
		\draw[arr] (stub |- ext.west)  -- (ext.west);
	\end{tikzpicture}

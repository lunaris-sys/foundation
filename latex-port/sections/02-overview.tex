...
\begin{tikzpicture}[
  ...
  tribox/.style={
    draw, thick, rectangle,
    minimum width=2.5cm, text width=2.3cm, minimum height=1.1cm,
...
% Top row: three consumer boxes
\node[tribox] (apps) at (-\triw, 4*\sep) {
  First-party\\Applications\\[-2pt]
  {\mdseries\footnotesize via os-sdk}
};
\node[tribox] (daemons) at (0, 4*\sep) {
  System\\Daemons\\[-2pt]
  {\mdseries\footnotesize Account, Health \ldots}
};
\node[optbox] (ai) at (\triw, 4*\sep) {
  AI Daemon\\[-2pt]
  {\mdseries\footnotesize optional}
};

% Arrows from consumers down to Knowledge Graph
...
The graph database is the system's long-term memory: a structured record of
everything that happens on the machine, queryable across time, application
...
searching in Spotlight, or checking System Monitor all translate to parameterized

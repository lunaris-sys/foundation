...
accounts. Set up once, available everywhere, with full visibility into which
application accesses what. The design principle is straightforward: no
application ever directly handles OAuth flows, stores tokens, or manages
credentials. All account logic lives in a dedicated \emph{Account Daemon}.
...
exits. All credentials at rest are stored in the Kernel Keyring; the daemon
...
Applications built for GNOME that use GNOME Online Accounts expect a D-Bus
...
\emph{Proton} is Valve's fork of Wine, optimized for gaming and maintained
with extensive patches from Valve's Linux gaming team; for gaming specifically
it is substantially better than vanilla Wine. Both are pre-installed alongside
\emph{DXVK}, which translates Direct3D 9, 10, and 11 calls to Vulkan, and
\emph{vkd3d-proton}, Valve's fork of vkd3d that translates Direct3D 12 calls
to Vulkan. Winetricks is included for installing Windows runtime components
into Wine prefixes.
...
\emph{Gamemode} by Feral Interactive is pre-installed and enabled by default.
It applies system-level performance optimizations when a game is running: the
CPU governor switches to performance mode, the I/O scheduler is tuned, and
kernel scheduler hints are applied. Proton handles the Gamemode activation call
automatically; no manual configuration is required.

\emph{MangoHud} is pre-installed as an in-game overlay showing frames per
second, GPU and CPU usage, temperatures, and frame timing. It is disabled by
default and toggled via a keyboard shortcut.
...
all modern x86 and ARM processors have hardware AES acceleration that makes
encryption effectively free at current workloads.

The primary unlock mechanism is a passphrase entered at boot. On systems with
a \emph{TPM}, the FDE key can be sealed to the TPM and released automatically
...
\emph{Secure Boot} is a UEFI firmware feature that verifies the cryptographic
...
bootloader, the kernel image, and the initramfs. A \emph{Unified Kernel Image}
...
\midrule
Linux Namespaces & Changes what the application can see \\
AppArmor         & Blocks actions on resources the application can see \\
...
No application has direct access to the Kuzu database files. All graph access
...
\paragraph{Kuzu file protection.}
The Kuzu database files are protected by three independent layers. At the
...
from memory. During any period when the Graph Daemon is not running, the Kuzu
...
components such as the AI layer and system daemons receive
...
HMAC signature on each query without a database lookup. This is faster at high
query rates, more robust against a corrupted ACL table, and allows the AI layer
to derive restricted sub-tokens for specific query scopes without involving the
daemon. Token revocation when permissions change mid-session is handled via
a small in-memory revocation list that is cleared on daemon restart.
...
\texttt{MemoryDenyWriteExecute}, \texttt{PrivateTmp}, and a tightly scoped
\texttt{SystemCallFilter}. \texttt{PrivateNetwork=true} means a compromised
daemon cannot make outbound connections to exfiltrate data. If the daemon is
compromised despite these controls, the blast radius is bounded to Kuzu data
only, and a systemd watchdog restart closes the LUKS image cleanly and starts
fresh.

Clients verify that they are speaking to the legitimate Graph Daemon before
sending queries. At startup, the daemon signs a challenge token with a private
key that only it holds; clients verify the signature against the public key
...
a burst limit of 200; the AI layer has a higher sustained limit appropriate
...
The AI layer has broader graph access than most applications by necessity,
...
2 & Project-scoped & All data associated with a user-selected project \\
...
The AI layer has no direct network access. All cloud API calls go through a
dedicated system proxy daemon that is the only component allowed to make
outbound connections to AI provider endpoints. The AI constructs a request,
hands it to the proxy, and receives only the response. This prevents a
compromised AI layer from independently exfiltrating data over arbitrary
...
network destinations are reachable only when a VPN is active. WireGuard is
the preferred VPN protocol given its in-kernel availability since Linux 5.6,
clean key management, and performance characteristics. OpenVPN support is
provided for compatibility with existing enterprise infrastructure.
...
\emph{USBGuard} enforces a policy on USB device connections. When the screen
...
\emph{IOMMU} restricts which memory regions each hardware device can access
via Direct Memory Access. With IOMMU enabled, a malicious Thunderbolt or
...
investigates. This is inspired by the BeyondCorp model and represents a
meaningful differentiator for enterprise deployments.
...
layer. Intel CET adds a hardware-enforced shadow stack, a separate protected
copy of return addresses that the CPU verifies on every function return.
Control-flow hijacking attacks that overwrite return addresses produce a
mismatch that causes an immediate fault. This is supported by the Linux kernel
since version 6.6 and is enabled by default for new processes on supported
hardware with no application changes required. For any C or C++ components
compiled as part of this project, Clang's CFI instrumentation is applied,
validating at runtime that indirect calls and virtual dispatch target only
...
directory. Each user has a separate Kuzu instance, a separate Graph Daemon
...
via standard LDAP and Kerberos protocols that existing enterprise infrastructure
already supports, requiring no proprietary agents on the management server side.
...
inspired by the BeyondCorp model and is a meaningful differentiator for

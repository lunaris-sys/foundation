...
HMAC signature on each query without a database lookup. \emph{HMAC} (Hash-based Message
Authentication Code) is a standard construction for verifying both the
integrity and authenticity of a message using a shared secret key; the verifier
recomputes the HMAC and checks it matches without needing to contact any
authority. This is faster at high query rates, more robust against a corrupted
ACL table, and allows the AI layer
